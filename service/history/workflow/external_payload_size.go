...
func CalculateExternalPayloadSize(events []*historypb.HistoryEvent, metricsHandler metrics.Handler) (size int64, count int64, err error) {
	...
	metricsHandler.Counter(metrics.ExternalPayloadUploadCount.Name()).Record(int64(totalCount))
	metricsHandler.Counter(metrics.ExternalPayloadUploadTotalSize.Name()).Record(int64(totalSize))
