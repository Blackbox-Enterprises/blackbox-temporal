...
func TestCalculateExternalPayloadSize_WithExternalPayloads(t *testing.T) {
	...
	countRecs := snapshot[metrics.ExternalPayloadUploadCount.Name()]
	require.Len(t, countRecs, 1)
	assert.Equal(t, int64(3), countRecs[0].Value)

	totalSizeRecs := snapshot[metrics.ExternalPayloadUploadTotalSize.Name()]
	require.Len(t, totalSizeRecs, 1)
	assert.Equal(t, int64(3584), totalSizeRecs[0].Value)
}
